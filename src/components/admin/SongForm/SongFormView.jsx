...
export default function SongFormView({
  song,
  albums,
  isEditing,
  navigate,
      ...
      .map((_, i) => {
        ...
        return (
          <motion.div
            ...
            animate={{ height: isPlaying ? [`${height}px`, `${Math.floor(Math.random() * 32) + 4}px`] : `${height}px` }}
            transition={{ duration: 0.4, repeat: isPlaying ? Infinity : 0, repeatType: "reverse" }}
          />
        );
      });
  };
      ...
      <form onSubmit={handleFormSubmit} className="relative">
                ...
                <div className="relative">
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      onBlur={() => handleTouch("track_number")}
                      className={`font-mono text-2xl text-center bg-indigo-950 border ${
                        validationErrors.track_number ? "border-red-500" : "border-indigo-700"
                      } rounded-lg shadow-inner py-2 px-4 w-24 focus:ring-2 focus:ring-indigo-400 focus:border-indigo-500`}
                      placeholder="##"
                      required
                    />
                    <div className="ml-3 flex flex-col">
                      <button
                        type="button"
                        onClick={() => handleChange("track_number", song.track_number ? parseInt(song.track_number) + 1 : 1)}
                        className="bg-indigo-800 hover:bg-indigo-700 rounded-t px-2 py-1"
                      >
                      ...
                      <button
                        ...
                        onClick={() =>
                          handleChange(
                            "track_number",
                            song.track_number && parseInt(song.track_number) > 1 ? parseInt(song.track_number) - 1 : 1
                          )
                        }
                        className="bg-indigo-800 hover:bg-indigo-700 rounded-b px-2 py-1 border-t border-indigo-950"
                      >
                        <svg className="w-4 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  {validationErrors.track_number && (
                    <p className="mt-1 text-sm text-red-400">{validationErrors.track_number}</p>
                  )}
                </div>
                ...
                <div className="relative">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="bg-black/40 px-3 py-1 rounded font-mono text-lg text-indigo-100">
        ...
        <div className="bg-black/30 backdrop-blur-sm rounded-xl border border-indigo-700/60 shadow-lg overflow-hidden">
          <div className="flex items-center justify-between p-5 border-b border-indigo-800/50">
            ...
            <div className="flex space-x-2">
              ...
              <button
                type="button"
                onClick={() => setActiveLyricsTab("translation")}
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                  activeLyricsTab === "translation"
                    ? "bg-indigo-600 text-white"
                    : "text-indigo-300 hover:bg-indigo-800/50"
                }`}
              >
            ...
            {song.lyrics && (
              <div className="flex gap-4 text-xs text-indigo-300 bg-black/20 rounded-full px-4 py-2">
                ...
                <span>
                  {activeLyricsTab === "original" ? lyricsStats.verses : Math.ceil(translationLines.length / 4)} verses
                </span>
                <span>
                  {activeLyricsTab === "original" ? lyricsStats.chars : (song.lyrics_translation || "").length} chars
                </span>
              </div>
            )}
          </div>
          ...
          <div className="relative">
            ...
            {activeLyricsTab === "original" ? (
              <TextareaAutosize
                ...
                className="font-mono leading-relaxed w-full px-6 py-6 rounded-lg border-0 bg-black/30 text-white focus:ring-2 focus:ring-indigo-400 focus:border-indigo-500 min-h-[350px] shadow-inner resize-none pl-14"
              ...
              <TextareaAutosize
                value={song.lyrics_translation || ""}
                onChange={(e) => handleChange("lyrics_translation", e.target.value)}
                className="font-mono leading-relaxed w-full px-6 py-6 rounded-lg border-0 bg-black/30 text-white focus:ring-2 focus:ring-indigo-400 focus:border-indigo-500 min-h-[350px] shadow-inner resize-none pl-14"
        ...
        <div className="bg-black/30 backdrop-blur-sm rounded-xl border border-indigo-700/60 shadow-lg overflow-hidden mt-6">
          ...
          <TextareaAutosize
            ...
            className="w-full px-6 py-4 rounded-lg border-0 bg-black/30 text-white focus:ring-2 focus:ring-indigo-400 focus:border-indigo-500 min-h-[100px] shadow-inner resize-none"
        ...
        <div className="flex justify-between items-center gap-4 pt-6 mt-8 px-6 py-4 bg-black/30 backdrop-blur-sm border-t border-indigo-700/50">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="h-10 w-10 rounded-full bg-indigo-800 hover:bg-indigo-700 flex items-center justify-center"
            >
              {isPlaying ? (
          ...
          <div className="flex items-center gap-4">
            ...
            <motion.button
              ...
              type="submit"
              disabled={hasErrors}
              className="px-6 py-3 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white shadow-lg shadow-indigo-700/50 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center gap-2 text-sm font-medium uppercase tracking-wide"
            >
              {isEditing ? (
                <>
