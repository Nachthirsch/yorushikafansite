...
const AlbumDetail = ({ album, onClose }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="relative bg-neutral-900 rounded-xl overflow-hidden max-w-4xl w-full max-h-[90vh] flex flex-col md:flex-row shadow-2xl">
        {/* Close button */}
        <button onClick={onClose} className="absolute top-4 right-4 text-neutral-400 hover:text-neutral-100 z-10 p-2">
        ...
        <div className="md:w-1/3 p-6 flex flex-col">
          ...
          <h2 className="text-2xl font-medium text-neutral-100 mb-2">{album.title}</h2>
          <p className="text-neutral-300 mb-4">Released: {new Date(album.release_date).toLocaleDateString()}</p>

          {album.classification && (
            <div className="mb-3">
              <span className="px-2 py-1 bg-neutral-700 text-neutral-200 rounded text-sm">{album.classification}</span>
            </div>
          )}

          <p className="text-neutral-300 text-sm mb-4">{album.description}</p>

          {/* External Links */}
          <div className="mt-auto flex flex-col space-y-2">
            {album.spotify_url && (
              <a href={album.spotify_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-[#1DB954]/20 hover:bg-[#1DB954]/30 text-neutral-100 py-2 px-4 rounded-full transition-colors">
            ...
            {album.youtube_url && (
              <a href={album.youtube_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-[#FF0000]/20 hover:bg-[#FF0000]/30 text-neutral-100 py-2 px-4 rounded-full transition-colors">
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className="md:w-2/3 bg-neutral-800/50 p-6 overflow-y-auto max-h-[500px] md:max-h-none">
          <h3 className="text-xl font-medium text-neutral-100 mb-4 flex items-center">
            <span>Tracks</span>
            <span className="ml-2 text-sm bg-neutral-700 px-2 py-0.5 rounded-full">{album.songs?.length || 0} songs</span>
          </h3>
